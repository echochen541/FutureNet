...
public final class AdvancedRoute {
	...
	public static void searchRoute(String graphContent, String condition) {
		...
		for (ArrayList<Integer> sp : AntColony.shortestPaths) {
			if (isValid(sp))
				System.out.println(sp);
		}

		/**
		 * ac1 = new AntColony(g, s, t, specifiedSet2); ac1.init(); StringBuffer
		 * sb2 = new StringBuffer(); for (int i = 0; i <
		 * AntColony.shortestPath.size() - 1; i++) {
		 * sb2.append(g.edgeIDs[AntColony.shortestPath.get(i)][AntColony.
		 * shortestPath.get(i + 1)] + "|"); }
		 * System.out.println(AntColony.shortestPathLength + ": " +
